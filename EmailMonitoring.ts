...
export class EmailMonitoring {
  ...
  static async cleanupOldData(daysToKeep: number = 90): Promise<void> {
    try {
      ...
      // Clean up alerts
      const supabaseAdminAlerts = getSupabaseAdmin();
      const { error: alertsError } = await supabaseAdminAlerts
        .from('email_alerts')
        .delete()
        .lt('created_at', cutoffDate.toISOString());
  ...
  static async getDashboardData(organizationId?: string, period: string = '30d') {
    try {
      if (organizationId) {
        ...
        const supabaseAdminOrgs = getSupabaseAdmin();
        const { data: orgs } = await supabaseAdminOrgs
          .from('organizations')
          .select('id, name');
