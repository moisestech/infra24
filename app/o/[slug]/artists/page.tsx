...
const roleColors = {
  resident: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
  associate: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
  staff: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
  alumni: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
  artist: 'bg-pink-100 text-pink-800 dark:bg-pink-900/20 dark:text-pink-400'
};
...
export default function ArtistsPage() {
  ...
  const filteredArtists = artists.filter(artist => {
    ...
    const matchesRole = !roleFilter || artist.role === roleFilter;
    const matchesStudio = !studioFilter || artist.studio === studioFilter;
    
    return matchesSearch && matchesRole && matchesStudio;
  });

  const uniqueStudios = [...new Set(artists.map(artist => artist.studio).filter(Boolean))];
  const uniqueRoles = [...new Set(artists.map(artist => artist.role))];
  ...
  const getRoleColor = (role: string) => {
    return roleColors[role as keyof typeof roleColors] || 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400';
  };
            ...
            <div key={artist.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="relative">
                ...
                <div className="absolute top-3 right-3">
                  <Badge className={`${getRoleColor(artist.role)} flex items-center space-x-1`}>
