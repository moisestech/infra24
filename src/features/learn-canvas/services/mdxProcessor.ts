...
class MDXProcessor {
  ...
  async processChapter(workshopSlug: string, chapterSlug: string): Promise<ProcessedMDX | null> {
    ...
    if (typeof window !== 'undefined') {
      console.warn('processChapter can only be called on the server side')
      return null
    }
    ...
    try {
      ...
      console.error(`Error processing chapter ${chapterSlug} for workshop ${workshopSlug}:`, error)
      return null
    }
  }
  ...
  async getWorkshopComponents(workshopSlug: string): Promise<MDXComponent[]> {
    // Only run on server side
    if (typeof window !== 'undefined') {
      console.warn('getWorkshopComponents can only be called on the server side')
      return []
    }
    ...
    try {
      ...
      console.error(`Error getting components for workshop ${workshopSlug}:`, error)
      return []
    }
  }
}
