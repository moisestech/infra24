// Simple script to enable learn content for Oolite workshops using API endpoints

const workshopIds = {
  ownYourDigitalPresence: '6ec503ab-7292-4459-95d1-7cf45ce95748',
  seoWorkshop: '11195c3d-37c2-4994-a8ce-615c68f45126'
};

const ownYourDigitalPresenceChapters = [
  {
...
async function setupOoliteLearnContent() {
  console.log('🎓 Setting up learn content for Oolite workshops...');
  
  try {
    // Enable learn content for "Own Your Digital Presence"
    console.log('📚 Enabling learn content for "Own Your Digital Presence"...');
    const enableResponse = await fetch(`http://localhost:3000/api/workshops/${workshopIds.ownYourDigitalPresence}/enable-learn`, {
      method: 'POST',
      headers: {
    ...
    if (!enableResponse.ok) {
      throw new Error(`Failed to enable learn content: ${enableResponse.status}`);
    }
    
    const enableResult = await enableResponse.json();
    console.log('✅ Learn content enabled:', enableResult.success);
    
    // Create chapters for "Own Your Digital Presence"
    console.log('📖 Creating chapters for "Own Your Digital Presence"...');
    const chaptersResponse = await fetch(`http://localhost:3000/api/workshops/${workshopIds.ownYourDigitalPresence}/chapters`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ chapters: ownYourDigitalPresenceChapters }),
    });
    
    if (!chaptersResponse.ok) {
      throw new Error(`Failed to create chapters: ${chaptersResponse.status}`);
    }
    
    const chaptersResult = await chaptersResponse.json();
    console.log('✅ Chapters created:', chaptersResult.success);
    console.log('📚 Created', chaptersResult.chapters?.length || 0, 'chapters');
    
    // Create chapters for "SEO Workshop"
    console.log('📖 Creating chapters for "SEO Workshop"...');
    const seoChaptersResponse = await fetch(`http://localhost:3000/api/workshops/${workshopIds.seoWorkshop}/chapters`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ chapters: seoWorkshopChapters }),
    });
    
    if (!seoChaptersResponse.ok) {
      throw new Error(`Failed to create SEO chapters: ${seoChaptersResponse.status}`);
    }
    
    const seoChaptersResult = await seoChaptersResponse.json();
    console.log('✅ SEO chapters created:', seoChaptersResult.success);
    console.log('📚 Created', seoChaptersResult.chapters?.length || 0, 'chapters');
    ...
    console.log(`   http://localhost:3000/learn/${workshopIds.ownYourDigitalPresence}`);
    console.log(`   http://localhost:3000/learn/${workshopIds.seoWorkshop}`);
    
  } catch (error) {
    console.error('❌ Error setting up learn content:', error);
  }
}

setupOoliteLearnContent();
